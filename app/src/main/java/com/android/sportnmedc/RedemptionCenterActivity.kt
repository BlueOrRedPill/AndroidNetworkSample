...
class RedemptionCenterActivity: BaseActivity(R.layout.activity_redemption_center),ToolbarHelper{
    ...
    companion object {
        fun newIntent(mActivity: AppCompatActivity): Intent {
            return Intent(mActivity,RedemptionCenterActivity::class.java)
...
//                tab_indicator.getTabAt(p0?.position?:0)?.setIcon(R.drawable.ic_triangle)?.select()
        ...
        reward_system.setOnClickListener {
            startActivity(RewardSystemActivity.newIntent(mActivity))
        }
    }
