...
class SellActivity:BaseActivity(R.layout.activity_sell_card),ToolbarHelper{
    ...
    companion object {
        fun newIntent(mActivity: AppCompatActivity): Intent {
            val intent = Intent(mActivity,SellActivity::class.java)
            return intent
        }
    }
    ...
    override fun onActivityCreated() {
        ...
        confirm_btn.setOnClickListener {
           val priceStr =  price_edt.text.toString()
            ...
            if (price > 0){
                startActivityForResult(SellConfirmActivity.newIntent(mActivity,price),Utils.REQUEST_CODE_BUYSELL)
            }else{
