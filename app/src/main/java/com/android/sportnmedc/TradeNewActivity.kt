...
class TradeNewActivity:BaseActivity(R.layout.activity_trade_new),ToolbarHelper{
    override fun hasBackMenu(): Boolean =true
    override fun toolbarTitle(): String? =""

    var mTradeModel = TradeModel(arrayListOf(0,0,0),arrayListOf(0,0,0),UserSource().getCurrentProfile())
    ...
    companion object {
        fun newIntent(mActivity: AppCompatActivity): Intent {
            val intent = Intent(mActivity,TradeNewActivity::class.java)
//            intent.putExtra("tradeModel",tradeModel)
    ...
    override fun onActivityCreated() {
        setupToolbar()
        UserSource().getCurrentProfile().let {
    ...
    fun updateContent(){
         ...
         cardGive.forEachIndexed{ index, imageView ->
             if ( mTradeModel.cardOut[index] == 0){
                 ...
                 imageView.setOnClickListener {
                     startActivityForResult(SelectCardActivity.newIntent(mActivity,mTradeModel,index,true),Utils.REQUEST_CODE_TRADE)
                 }
             }else{
        ...
        cardNeed.forEachIndexed { index, imageView ->
            if (mTradeModel.cardIn[index] == 0){
                ...
                imageView.setImageResource(R.drawable.img_add_card)
                imageView.setOnClickListener {
                    startActivityForResult(SelectCardActivity.newIntent(mActivity,mTradeModel,index,false),Utils.REQUEST_CODE_TRADE)
                }
            }else{
