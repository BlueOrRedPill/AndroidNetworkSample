...
class FindUserActivity:BaseActivity(R.layout.activity_find_user),ToolbarHelper{
    ...
    companion object {
        fun newIntent(mActivity:AppCompatActivity):Intent{
            val intent = Intent(mActivity,FindUserActivity::class.java)
        ...
        mUserAdapter?.setOnItemClick(object:BaseRecyclerAdapter.OnItemClicked{
            override fun onItemClicked(v: View, position: Int) {
                mUserAdapter?.getItem(position)?.let {
                    startActivityForResult(UserDetailActivity.newIntent(mActivity,it), Utils.REQUEST_CODE_TRADE)
                }
            }
