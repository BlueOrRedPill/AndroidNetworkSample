...
import com.android.sportnmedc.fragments.*
...
class HomeActivity:BaseActivity(R.layout.activity_home),ToolbarHelper{
    ...
    override fun initMenuEvent(mToolbar: Toolbar?) {
        privacy_btn.setOnClickListener {
            startActivity(TextActivity.newIntent(mActivity,"Privacy Policy",true))
        }
        term_btn.setOnClickListener {
            startActivity(TextActivity.newIntent(mActivity,"Terms and Condition",true))
        }
        change_pass_btn.setOnClickListener {
            startActivity(TextActivity.newIntent(mActivity,"Change Password",false))
        }
        ...
        mToolbar?.rightMenuTextEvent("300",View.OnClickListener {

        })
        buy_token_btn.setOnClickListener { startActivity(PaymentActivity.newIntent(mActivity)) }
    ...
    private fun selectFragment(index:Int){
        ...
        when(index){
            ...
            1 -> {
                ...
                mFragment = LibraryFragment()
            }
            2 -> {
                mToolbar?.setToolbarTitle("MARKETS")
                mFragment = MarketFragment()

            }
            3 -> {
                mToolbar?.setToolbarTitle("TRADE")
                mFragment = TradeFragment()
            }
            4 -> {
